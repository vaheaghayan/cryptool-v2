<nav>
    <div style="z-index: 2" class="w3-bar caret-gray-500 bg-dark  w3-card w3-left-align w3-large w3-top">
        ...
        @foreach(\App\Models\Cypher::CATEGORIES as $category)
        ...
        ">{{__('About')}}</a>
        <a href="{{url(cLng(). '/forum')}}" class="w3-bar-item text-light w3-button w3-hide-small w3-padding-large w3-hover-white">
            {{__('Forum')}}
        </a>
        ...
        <a href="#" class="w3-bar-item text-light  w3-right w3-button w3-hide-small w3-padding-large w3-hover-white">{{__(strtoupper(cLng()))}}</a>
        @else
